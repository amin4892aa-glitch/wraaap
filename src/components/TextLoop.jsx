...
const TextLoop = ({
  ...
  return (
    <div ref={rootRef} className={`text-loop ${className}`.trim()} style={style}>
      <svg className="text-loop-svg" viewBox={`0 0 ${VIEW_W} ${VIEW_H}`} aria-hidden="true">
        <defs>
        ...
        <text className="text-loop-text" fill={color} style={textStyle}>
          <textPath ref={headRef} href={`#${pathId}`} lengthAdjust="spacing" textLength={fitLength}>
            {loopText}
          </textPath>
        </text>
        <text className="text-loop-text" fill={color} style={textStyle}>
          <textPath ref={tailRef} href={`#${pathId}`} lengthAdjust="spacing" textLength={fitLength}>
            {loopText}
          </textPath>
        </text>
      </svg>
